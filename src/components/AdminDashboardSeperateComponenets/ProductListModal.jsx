...
//   return (
//     <div
//       className={`fixed inset-0 z-[70] flex items-center justify-center  bg-opacity-40 backdrop-blur-sm transition-opacity duration-300 p-4`}
...
//     // Parse the input as integer (default to 0 if empty or invalid)
...
//         [size]: newStock,
//       },
//       newSizeStock: "", // Clear input after update
//     }));
//   };
...
//     // 1. Prepare size data and calculate total stock
...
//     // Call the parent's update function (which handles Firestore)
...
//                 <div
//                   key={size}
//                   className="bg-white p-2 border rounded-md shadow-sm text-center flex-1 min-w-[70px]"
//                 >
//                   <p className="text-xs font-medium text-gray-500">
//                     {size} Size
//                   </p>
//                   <p className="text-lg font-bold text-gray-800">
//                     {formData.currentSizes[size]}
//                   </p>
//                 </div>
//               ))}
//             </div>
...
//               To update stock, input the new total count above and click the "+"
//               button.
//             </p>
//           </div>
...
// // --- 3. Product Table Row Component ---

// const ProductTableRow = ({ product, onDelete, onEdit }) => {
//   const [isConfirmModalOpen, setIsConfirmModalOpen] = useState(false);

//   // Determine stock status color (product.stock is now the total stock)
//   const stockColor =
//     product.stock < 10
...
//       {/* ID */}
//       <td className="px-4 py-3 text-xs font-mono text-gray-500 truncate hidden md:table-cell">
//         {product.id}
//       </td>
...
//         <span
//           className={`px-2 py-1 text-xs font-semibold rounded-full ${stockColor}`}
//         >
//           {product.stock} pcs
//         </span>
...
//             className="p-2 text-blue-500 rounded-lg hover:bg-blue-100 transition duration-200"
//             title="Edit Product"
//           >
//             <Pencil size={16} />
//           </button>
...
//             className="p-2 text-red-500 rounded-lg hover:bg-red-100 transition duration-200"
...
//   useEffect(() => {
//     axios
//       .get(`${import.meta.env.VITE_PRODUCT_URL}/get-product-list`)
//       .then((res) => {
...
//     try {
//       await axios.put(
//         `${import.meta.env.VITE_PRODUCT_URL}/update-product`,
//         updateData
//       );
//       setProductList((prevList) =>
//         prevList.map((p) => (p.id === updateData.id ? updateData : p))
//       );
...
//     try {
//       await axios.delete(
//         `${import.meta.env.VITE_PRODUCT_URL}/delete-product/${id}`
//       );
...
//     // Modal Overlay
//     <div
//       className={`backdrop-blur-sm  fixed inset-0 bg-opacity-70 z-50 ${display} items-center justify-center p-4`}
//     >
//       {/* Product Dashboard Container */}
//       <div
//         className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col transform transition-transform duration-300"
//         onClick={(e) => e.stopPropagation()} // Prevent click-to-close on modal body
//       >
//         {/* === Card Header === */}
...
//                 Manage the list of available products in real-time. Total items:{" "}
//                 {}.
//               </p>
//             </div>
//             <button
//               onClick={() => setProductListModal(false)}
...
//             {/* Action Buttons (Placeholder for adding new product) */}
//             <div className="flex gap-3 shrink-0">
//               <button
//                 className="bg-blue-600 text-white flex items-center gap-2 px-4 py-2 rounded-lg font-semibold shadow-md hover:bg-blue-700 transition duration-300"
...
//               No products match your current filter or have been added yet. (No
...
//                     Price & Discount
//                   </th>
//                   <th className="px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider text-center hidden sm:table-cell ">
//                     Stock
//                   </th>
//                   <th className="px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider text-right">
//                     Actions
...
//                   .filter(
//                     (p) =>
//                       p.name.toLowerCase().includes(searchTerm.toLowerCase()) || // name match
//                       p._id.includes(searchTerm) // ID match
//                   )
//                   .map((product) => (
//                     <ProductTableRow
//                       key={product._id} // _id is safer than id
//                       product={product}
...
//             <button
//               disabled={true}
//               className="px-3 py-1 text-sm border border-gray-300 rounded-lg text-gray-700 disabled:opacity-50 transition"
//             >
//               Previous
//             </button>
//             <button
